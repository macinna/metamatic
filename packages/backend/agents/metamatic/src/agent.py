...
# Import shared types and config (will work once UV workspace is set up)
# from shared.config import AgentConfig
# from shared.types import CustomerInfo, OrderInfo
# from shared.utils import safe_json_dumps, setup_logging
...
def get_customer_id(email_address: str) -> str:
    ...
    if email_address == "me@example.net":
        response = {"customer_id": 123}  # type: ignore
    else:
        response = {"message": "customer not found"}  # type: ignore
    try:
...
def get_orders(customer_id: int) -> str:
    ...
    if customer_id == 123:
        ...
        response = {"message": "no order found"}  # type: ignore
    try:
        return json.dumps(response)
    except Exception as e:
        return str(e)
    ...
    if "smartphone" in topic:
        if "cover" in topic:
            response_items.append(
                "To put on the cover, insert the bottom first, then push from the back up to the top."
            )
            response_items.append(
                "To remove the cover, push the top and bottom of the cover at the same time."
            )
        if "charger" in topic:
            response_items.append("Input: 100-240V AC, 50/60Hz")
            response_items.append("Includes US/UK/EU plug adapters")

    if response_items:
        response = {"info": response_items}  # type: ignore
    else:
        response = {"message": "no info found"}  # type: ignore
